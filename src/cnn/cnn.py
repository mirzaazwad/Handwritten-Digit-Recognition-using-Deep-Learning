...
class DataLoader:
    def load_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        mnist_data = dataset.data.to_numpy().reshape((-1, 28, 28, 1)) / 255.0
...
class ModelBuilder:
    def build(self, save_weights_path: Optional[str] = None):
        model = cnn.CNN.build(
            width=28, height=28, depth=1, total_classes=10, save_weights_path=save_weights_path
        )
...
class Trainer:
    def train(self, model, train_img, train_labels, batch_size=128, epochs=20):
        print("Training model...")
        return model.fit(train_img, train_labels, batch_size=batch_size, epochs=epochs, verbose=1)

    def evaluate(self, model, test_img, test_labels):
...
class Predictor:
    def predict_and_display(self, model, test_img, test_labels, show=False):
...
def main(save_model_flag=-1, load_model_flag=-1, save_weights_path="cnn_model.h5"):
    ...
    train_img, train_labels, test_img, test_labels = data_loader.load_data()
    train_labels_cat = np_utils.to_categorical(train_labels, 10)
    test_labels_cat = np_utils.to_categorical(test_labels, 10)

    model = model_builder.build(save_weights_path if load_model_flag > 0 else None)

    if load_model_flag < 0:
        trainer.train(model, train_img, train_labels_cat)

    trainer.evaluate(model, test_img, test_labels_cat)

    if save_model_flag > 0:
        trainer.save_model(model, save_weights_path)
    predictor.predict_and_display(model, test_img, test_labels_cat, show=False)
